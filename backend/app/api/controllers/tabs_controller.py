...
async def get_dashboard_tabs(dashboard_id: int, service: tabs_service):
    return await service.get_dashboard_tabs(dashboard_id)


async def get_tab_by_id(tab_id: int, service: tabs_service):
    return await service.get_tab_by_id(tab_id)


async def get_tab_max_index(dashboard_id: int, service: tabs_service):
    return await service.get_tab_max_index(dashboard_id)


async def create_tab(dashboard_id: int, body: TabCreate, service: tabs_service):
    return await service.create_tab(dashboard_id, body.tab_name)


async def update_tab(tab_id : int, body: TabUpdate, service: tabs_service):
...
async def delete_tab(tab_id : int, service: tabs_service):
